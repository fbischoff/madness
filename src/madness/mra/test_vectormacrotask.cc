...
//			Function<double,4> g=real_factory_4d(world).functor(gaussian(0.5));
...
public:
	...
	void run_all(std::vector<std::shared_ptr<MacroTaskBase> >& vtask) {
		...
		while (true){
			...
			print("starting task ",element);
                        
			task->run(subworld,cloud);
			print("ended task ",element);
//			subworld.gop.fence();
...
//    madness::World& universe = madness::initialize(argc,argv);
...

    {
		...
		universe.gop.fence();
		{
			Function<double,4> ff(taskq.get_subworld());
			print("world exists",taskq.get_subworld().id(),World::exists(&taskq.get_subworld()));
			taskq.cloud.load(taskq.get_subworld(),ff,0);
		}
